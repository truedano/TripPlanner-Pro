...
import { MapPin, Car, Bed, Utensils, GripVertical, Trash2, Clock, Copy } from 'lucide-react';
...
export const SortableSpotItem: React.FC<SortableSpotItemProps> = ({
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`group border rounded-[2.5rem] p-4 sm:p-5 hover:shadow-xl transition-all hover:bg-white select-none ${theme.bgColor} border-slate-100 hover:border-blue-200`}
        >
            <div className="flex items-start">
                ...
                <div onClick={onClick} className="flex flex-grow items-start min-w-0 cursor-pointer">
                    <div className="flex flex-col items-center justify-center w-12 sm:w-24 pr-3 sm:pr-4 border-r border-slate-100 sm:border-slate-200 mr-3 sm:mr-6 shrink-0">
                        <span className={`text-[10px] sm:text-xs font-black ${theme.color}`}>{spot.startTime || '--:--'}</span>
                        <div className={`h-3 sm:h-4 w-0.5 my-0.5 sm:my-1 opacity-20 ${theme.color.replace('text-', 'bg-')}`}></div>
                    ...
                    <div className="flex-grow min-w-0">
                        <div className="flex justify-between items-start gap-2 mb-2">
                            <div className="flex flex-col min-w-0 flex-1">
                                <span className={`text-[10px] font-black uppercase tracking-tighter mb-1 ${theme.color}`}>{theme.label}</span>
                                <h4 className="font-black text-base sm:text-lg text-slate-800 break-words leading-tight">{spot.name || `未命名${theme.label}`}</h4>
                            </div>

                            <div className="flex items-center shrink-0 space-x-1">
                                {spot.type === SpotType.STAY && onCopy && (
                                    <button
                                        type="button"
                                        onClick={(e) => {
                                            e.stopPropagation();
                                        ...
                                        className="p-2 text-slate-300 hover:text-purple-600 transition-colors rounded-full hover:bg-purple-50"
                                        title="複製此住宿到其他日期"
                                    >
